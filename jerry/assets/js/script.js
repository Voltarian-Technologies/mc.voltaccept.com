...
// Fetch the rarity.json from your website
async function loadRarityColors() {
  try {
    const res = await fetch("https://or.voltaccept.com/jerry/api/rarity.json"); // same-site API
    if (!res.ok) throw new Error("Failed to load RarityColors.json");

    rarityColors = await res.json();
    console.log("RarityColors loaded:", rarityColors);
  } catch (err) {
    console.error("RarityColors fetch error:", err);
    rarityColors = {}; // fail-safe
  }
}
